...
void matrix_multi2(int n,float *matA,float *matB,float *mat_temp, float *mat_store){ //Pre-elem , Post-elem, aux , mat_store
	int i,j,k;
	...
	for (i = 1 ; i <= n ; i++ ){ 				
    	for (j = 1 ; j <= n ; j++ ){
    		mat_temp[i*n+j] = 0;
        	for (k = 1 ; k <= n ; k++ ){
	            mat_temp[i*n+j] += matA[k*n+i] * matB[k*n+j];
			}
		}
	}
	...
	for (i = 1 ; i <= n ; i++ ){ 				
    	for (j = 1 ; j <= n ; j++ ){
        	for (k = 1 ; k <= n ; k++ ){
	            mat_store[i*n+j] = mat_temp[i*n+j] ;
			}
		}
	}
}
...
void new_T_mat(int max_i, int max_j,int n,float *mat,float *T,float *mat_temp,float *eigvec){
  ...
  //           printf("[%f]",T[i*n+j]);
 	// }
    matrix_multi2(n,eigvec,T,mat_temp,eigvec); //Eigenvec calculation
    matrix_multi2(n,T,mat,mat_temp,mat); //Premultiply T
	matrix_multi2(n,mat,T,mat_temp,mat);//Postmultiply T
