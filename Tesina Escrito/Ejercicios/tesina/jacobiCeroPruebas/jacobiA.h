...
void mat_mult(int n,float *A, float *B,float *C){
	int i,j,k; 
	//#pragma acc data copy(A), copy(B), copy(C)
	for (i = 0 ; i < n ; i++ ){ 
...
/*****************************+*******************************/	
	...
	for (*nrot = 0; min == false ; ++*nrot){
		...
		//printf("Piv element = %f\n",mat[piv_elem[0]*n+piv_elem[1]]);

		for (i = 0; i < n; ++i){
			
		}

		if(fabs(mat[piv_elem[0]*n+piv_elem[1]]) < EPS || *nrot >= 1000000 ) //if max element doesnt exist more
			min=true;	
