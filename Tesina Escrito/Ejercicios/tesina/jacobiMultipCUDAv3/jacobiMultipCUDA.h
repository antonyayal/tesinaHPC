...
__global__ void new_T_mat(int max_i, int max_j,int n,float *mat,float *T){
...
/*****************************+*******************************/	
	...
	cudaMalloc(&d_piv_1,sizeof (int));
	cudaMalloc(&d_piv_2,sizeof (int));
	...
	for (*nrot = 0; min == false ; ++*nrot){
		...
		if(fabs(mat[piv_elem[0]*n+piv_elem[1]]) < EPS || *nrot >= 100 ) //if max element doesnt exist more
			...
			cudaMemcpy(d_piv_1, &piv_elem[0], sizeof (int), cudaMemcpyHostToDevice);
			cudaMemcpy(d_piv_2, &piv_elem[1], sizeof (int), cudaMemcpyHostToDevice);
			new_T_mat<<<1,1>>>(piv_elem[0],piv_elem[1],d_n,d_mat,d_T); //Calculate T matrix
